...
export async function populateAcademy(
  ctx: MutationCtx,
  ...
  for (const table of ACADEMY_TABLES) {
    const rows = await ctx.db
      .query(table)
      .filter((q) => q.eq(q.field("academyId"), academyId))
      .collect();
  ...
  return {
    ...
    monthsSeeded: months.length,
  };
}

export const seedAcademy = mutation({
  args: {
  ...
  async handler(ctx, args) {
    ...
    if (!academyId) {
      const academies = await ctx.db
        ...
        .collect();
      const valid = academies.filter((a) => a.deletedAt === undefined);
      if (valid.length === 0) throw new Error("No active academy found to seed");
      valid.sort((a, b) => b._creationTime - a._creationTime);
      academyId = valid[0]._id;
    }

    const academy = await ctx.db.get(academyId);
    if (!academy) throw new Error("Academy not found");

    const summary = await populateAcademy(ctx, academyId);
    return {
      academyId,
